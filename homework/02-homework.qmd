...
# Simulate where sex = 1, and where sex = 2
post_pred_sex_1 <- predicted_draws(m_b, DT_pred[, .(age, sex = 1)])
post_pred_sex_2 <- predicted_draws(m_b, DT_pred[, .(age, sex = factor(2, levels =c(1, 2)))])
...
ggplot(DT_diff, aes(age, diff)) +
	...
	labs(
		y = 'predicted difference in weight',
		title = paste('sex factor Posterior predicted difference in weight between',
									key[sex == 1, sex_char], 'and',
									key[sex == 2, sex_char])
	)


# Note: differences with epred
# Simulate where sex = 1, and where sex = 2
epred_sex_1 <- epred_draws(m_b, DT_pred[, .(age, sex = 1)])
epred_sex_2 <- epred_draws(m_b, DT_pred[, .(age, sex = 2)])

# Predicted table
DT_diff <- data.table(
	epred_1 = epred_sex_1$.epred,
	epred_2 = epred_sex_2$.epred,
	age = epred_sex_1$age
)

# Posterior of differences
DT_diff[, diff := epred_1 - epred_2]
...
ggplot(melt_samples, aes(factor(index_occasion + 1, levels = seq.int(8) + 1), value)) + 
	stat_slab() + 
	stat_dots(aes(y = diff_height), data = DT_ox, color = 'black', alpha = 0.2) + 
